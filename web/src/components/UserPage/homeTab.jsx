...
// --- NEW COMPONENT: OrderDeliveryPath ---
function OrderDeliveryPath({ activeOrders, userLatLng }) {
    // State to store the most recent courier locations fetched from Firestore
    const [liveCourierLocations, setLiveCourierLocations] = useState({});

    // Get the list of confirmed orders and extract relevant data
    ...
    // Effect to set up the polling interval for courier locations
    useEffect(() => {
        // Collect all unique courier IDs from the confirmed orders
    ...
    try {
        ...
        // 1. Correct Path: Only reference the document (Collection/Document = 2 segments)
        const docRef = doc(db, 'couriers', courierId); 
        
        const docSnap = await getDoc(docRef);
        
        // 2. Access the field 'location' from the document data
        const fetchedData = docSnap.exists() ? docSnap.data()?.location : null; // Access .location field
        
        // The structure of 'fetchedData' should now be the object 
        // { latitude: ..., longitude: ... }
        ...
        // Fetch immediately on mount/update, then set up the interval
        updateAllCouriers();
        
        const intervalId = setInterval(updateAllCouriers, 5000); // Poll every 5000 ms (5 seconds)
    ...
    return (
        <>
            {confirmedOrders.map((order) => {
                ...
                const userCoords = userLatLng; // User's location is passed as a prop
                
                // Retrieve LIVE courier coordinates, falling back to the initial data if live data isn't ready
                ...
                // 2. Determine Line Color (same as restaurant border)
                const pathColor = stringToColor(order.orderId);

                // The Polyline connects the three points: Restaurant -> Courier -> User
                    ...
                    <React.Fragment key={order.orderId}>
                      ...
                      {/*
                        <Polyline 
                            ...
                            pathOptions={{ color: pathColor, weight: 3, dashArray: '10, 5' }} // Dashed line for visual distinction //
                        /> */}
                        <CircleMarker
                            center={courierCoords} // Use the live coordinates for the marker
                            radius={8}
